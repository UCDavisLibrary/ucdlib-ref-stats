...
return html`
  ...
  <div class='teaser-grid'>
    ${[1,2,3].map(() => html`
      <phantom-ui>
        ${_renderTeaser.call(this)}
      </phantom-ui>
    `)}
    ${this.forms.map(form => _renderTeaser.call(this, form))}
  </div>
`;}

function _renderTeaser(form={}) {
  return html`
    <a href='/form/${form.name}' class="tile-link category-brand--${form.form_display_settings?.brandColor || 'primary'} category-brand__background">
      <div class="tile-link__title">
        <h3 class="tile-link__title-heading">${form.label || 'No Title'}</h3>
      </div>
